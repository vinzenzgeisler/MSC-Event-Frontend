...
export function MarshalAufbauView({ workspace, area, canWrite, busy, onPersonOpen, onAddPerson, onRemovePerson, onSaveShift }: Props) {
  ...
  return <>
    <Card><CardHeader className="p-4 sm:p-6"><CardTitle>{area.name}</CardTitle>{area.responsibleLabel && <p className="mt-1 text-sm text-slate-600">Verantwortlich: {area.responsibleLabel}</p>}</CardHeader>
      <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
        ...
        })}<td className="p-3">{person.participation.shirtSizeSnapshot ?? person.shirtSize ?? "—"}</td>{canWrite && <td className="p-2"><Button type="button" size="sm" variant="ghost" className="text-red-700" disabled={busy} aria-label={`${person.firstName} ${person.lastName} aus ${area.name} entfernen`} onClick={() => setRemovePerson(person)}><Trash2 className="h-4 w-4" /></Button></td>}</tr>)}</tbody></table>{people.length === 0 && <p className="p-6 text-center text-sm text-slate-500">Noch keine Personen in diesem Bereich.</p>}</div>
      </CardContent></Card>
    {removePerson && <MarshalConfirmDialog title="Bereichszuordnung entfernen?" description={<><strong>{removePerson.firstName} {removePerson.lastName}</strong> wird aus „{area.name}“ entfernt. Alle zugehörigen Aufbau-Schichtzuweisungen werden ebenfalls gelöscht.</>} confirmLabel={<><Trash2 className="mr-2 h-4 w-4" />Aus Bereich entfernen</>} confirmDisabled={busy} onConfirm={() => void confirmRemove()} onCancel={() => setRemovePerson(null)} />}
  </>;
}
