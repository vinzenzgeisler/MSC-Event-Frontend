...
type Props = {
  ...
  onRemovePerson: (person: MarshalPerson) => Promise<boolean>;
  onSaveShift: (person: MarshalPerson, shiftId: string, status: MarshalCommitmentStatus) => Promise<boolean>;
};

export function MarshalAufbauView({ workspace, area, canWrite, busy, onPersonOpen, onAddPerson, onRemovePerson, onSaveShift }: Props) {
  ...
  const people = areaMembers.filter((person) => statusFilter === "all" || workspace.areaAssignments.some((assignment) => assignment.areaId === area.id && assignment.participationId === person.participation.id && assignment.commitmentStatus === statusFilter) || workspace.shiftAssignments.some((assignment) => assignment.participationId === person.participation.id && shiftIds.has(assignment.shiftId) && assignment.commitmentStatus === statusFilter)).sort((a, b) => a.lastName.localeCompare(b.lastName, "de") || a.firstName.localeCompare(b.firstName, "de"));
  ...
  async function confirmRemove() {
    ...
    const removed = await onRemovePerson(removePerson);
    ...
    <>
      <Card>
        <CardHeader className="p-4 sm:p-6">
          ...
          {area.responsibleLabel && <p className="mt-1 text-sm text-slate-600">Verantwortlich: {area.responsibleLabel}</p>}
        </CardHeader>
        <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
          <div className="flex flex-col gap-2 rounded-xl border bg-slate-50 p-3 sm:flex-row sm:items-end">
            <label className="grid flex-1 gap-1 text-xs font-medium text-slate-600">
              Person hinzufügen
              <select className="h-10 rounded-md border bg-white px-3 text-sm font-normal text-slate-950" value={personId} disabled={!canWrite || busy} onChange={(event) => setPersonId(event.target.value)}>
            ...
            <Button type="button" disabled={!personId || !canWrite || busy} onClick={() => void addPerson()}>
          ...
          <div className="max-w-full overflow-x-auto overscroll-x-contain rounded-lg border">
                ...
                {people.map((person) => (
                  <tr key={person.id} className={cn("border-b", person.noDeployment && "bg-red-50 text-red-900")}>
                    ...
                    {shifts.map((shift) => {
                      ...
                      const conflicts = !["not_asked", "declined"].includes(status)
                        ? sameDayConflicts(workspace, person, shift.shiftDate, {
                            shiftId: shift.id,
                            areaId: area.id,
                          })
                        : [];
                      ...
                      return (
                        <td key={shift.id} className="w-44 min-w-44 p-2 text-center">
                          <span className="inline-flex w-full items-center justify-center">
                            ...
                            <DoubleBookingWarning labels={conflicts} day={conflictDay} />
                          </span>
                        </td>
                      );
                    })}
            ...
            {people.length === 0 && <p className="p-6 text-center text-sm text-slate-500">Noch keine Personen in diesem Bereich.</p>}
          </div>
        </CardContent>
      </Card>
      {removePerson && (
        <MarshalConfirmDialog
          ...
          description={
            <>
              ...
              wird aus „{area.name}“ entfernt. Alle zugehörigen Aufbau-Schichtzuweisungen werden ebenfalls gelöscht.
            </>
          }
          confirmLabel={
            <>
...
function statusClass(status: MarshalCommitmentStatus) {
  return status === "accepted" ? "border-green-300 bg-green-100 text-green-800" : status === "declined" ? "border-red-300 bg-red-100 text-red-800" : status === "pending" || status === "tentative" ? "border-amber-300 bg-amber-100 text-amber-800" : "border-slate-200 bg-slate-100 text-slate-500";
}
