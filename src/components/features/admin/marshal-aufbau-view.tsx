...
type Props = {
  ...
  onSave: (person: MarshalPerson, shiftId: string, status: MarshalCommitmentStatus) => Promise<boolean>;
};

export function MarshalAufbauView({ workspace, area, canWrite, busy, onPersonOpen, onSave }: Props) {
  ...
  async function addPerson() {
    ...
    if (!person || !shifts[0]) return;
    const saved = await onSave(person, shifts[0].id, "not_asked");
