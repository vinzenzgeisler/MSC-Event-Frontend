...
export function MarshalGeneralView({ workspace, area, dayKey, canWrite, busy, onDayChange, onPersonOpen, onSave, onRemove }: Props) {
  ...
  return <><Card><CardHeader className="p-4 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>Allgemeine Helfer</CardTitle>{area.responsibleLabel && <p className="mt-1 text-sm text-slate-600">{area.responsibleLabel}</p>}</div><div className="inline-flex self-start rounded-lg border bg-slate-50 p-1" role="group" aria-label="Veranstaltungstag">{(["saturday", "sunday"] as const).map((key) => <Button key={key} type="button" size="sm" variant={dayKey === key ? "default" : "ghost"} aria-pressed={dayKey === key} onClick={() => onDayChange(key)}>{key === "saturday" ? "Samstag" : "Sonntag"}</Button>)}</div></div></CardHeader>
