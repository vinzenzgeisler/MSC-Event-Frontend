...
export function MarshalGeneralView({ workspace, area, dayKey, canWrite, busy, onDayChange, onPersonOpen, onSave }: Props) {
  const [personId, setPersonId] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const assignments = workspace.areaAssignments.filter((item) => item.areaId === area.id);
  const people = workspace.people.filter((person) => assignments.some((assignment) => assignment.participationId === person.participation.id)).filter((person) => statusFilter === "all" || assignments.some((assignment) => assignment.participationId === person.participation.id && assignment.commitmentStatus === statusFilter)).sort((a, b) => a.helperNumber - b.helperNumber);
  const available = workspace.people.filter((person) => person.isActive && !person.noDeployment && !people.some((item) => item.id === person.id));
