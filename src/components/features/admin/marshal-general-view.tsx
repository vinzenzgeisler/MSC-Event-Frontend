...
export function MarshalGeneralView({ workspace, area, dayKey, canWrite, busy, onDayChange, onPersonOpen, onSave, onRemove }: Props) {
  ...
  const people = workspace.people
    .filter((person) => memberParticipationIds.has(person.participation.id))
    .filter((person) => statusFilter === "all" || assignments.some((assignment) => assignment.participationId === person.participation.id && assignment.commitmentStatus === statusFilter))
      ...
      <Card>
        <CardHeader className="p-4 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle>Allgemeine Helfer</CardTitle>
              {area.responsibleLabel && <p className="mt-1 text-sm text-slate-600">{area.responsibleLabel}</p>}
            </div>
            <div className="inline-flex self-start rounded-lg border bg-slate-50 p-1" role="group" aria-label="Veranstaltungstag">
              {(["saturday", "sunday"] as const).map((key) => (
                <Button key={key} type="button" size="sm" variant={dayKey === key ? "default" : "ghost"} aria-pressed={dayKey === key} onClick={() => onDayChange(key)}>
        ...
        <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
          <div className="grid gap-2 rounded-xl border bg-slate-50 p-3 sm:grid-cols-[minmax(0,1fr)_auto_220px] sm:items-end">
            <label className="grid gap-1 text-xs font-medium text-slate-600">
              Person hinzufügen
              <select className="h-10 rounded-md border bg-white px-3 text-sm font-normal" value={personId} disabled={!canWrite || busy} onChange={(event) => setPersonId(event.target.value)}>
                <option value="">Person wählen …</option>
                {available.map((person) => (
                  <option key={person.id} value={person.id}>
                    {person.helperNumber} · {person.lastName}, {person.firstName}
                  </option>
                ))}
              </select>
            </label>
            <Button type="button" disabled={!canWrite || busy || !personId} onClick={() => void addPerson()}>
              <UserPlus className="mr-2 h-4 w-4" />
              Hinzufügen
            </Button>
            <label className="grid gap-1 text-xs font-medium text-slate-600">
              Statusfilter
              <select className="h-10 rounded-md border bg-white px-3 text-sm font-normal" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[720px] w-full text-sm">
              ...
              <tbody>
                {people.map((person) => {
                  const assignment = assignments.find((item) => item.participationId === person.participation.id);
                  const status = assignment?.commitmentStatus ?? "not_asked";
                  const currentDay = workspace.days.find((item) => item.dayKey === dayKey);
                  const conflicts = currentDay && !["not_asked", "declined"].includes(status) ? sameDayConflicts(workspace, person, currentDay.eventDate, { areaId: area.id }) : [];
                  return (
                    <tr key={person.id} className={cn("border-b", person.noDeployment && "bg-red-50 text-red-900")}>
                      <td className="p-3 text-slate-500">{person.helperNumber}</td>
                      <td className="p-3">
                        ...
                        {person.noDeployment && <span className="ml-2 text-xs font-semibold">⚠ kein Einsatz</span>}
                      </td>
                      <td className="p-3">
                        <span className="inline-flex items-center">
                          <select aria-label={`Status für ${person.firstName} ${person.lastName}`} className="h-10 rounded-md border bg-white px-2" value={status} disabled={!canWrite || busy || person.noDeployment} onChange={(event) => void onSave(person, event.target.value as MarshalCommitmentStatus, assignment?.note ?? null)}>
                            {Object.entries(marshalStatusLabels).map(([value, label]) => (
                              <option key={value} value={value}>
                                {label}
                              </option>
                          ...
                          <DoubleBookingWarning labels={conflicts} day={currentDay} />
                        </span>
                      </td>
                      <td className="p-3">
                        <input
                          ...
                          disabled={!canWrite || busy || person.noDeployment}
                          onBlur={(event) => {
                            if (event.target.value !== (assignment?.note ?? "")) void onSave(person, status, event.target.value || null);
                          }}
                        />
                      </td>
                      {canWrite && (
            ...
            {people.length === 0 && <p className="p-6 text-center text-sm text-slate-500">Noch keine allgemeinen Helfer für {dayKey === "saturday" ? "Samstag" : "Sonntag"}.</p>}
          </div>
        </CardContent>
      </Card>
      {removePerson && (
        <MarshalConfirmDialog
          title="Bereichszuordnung entfernen?"
          description={
            <>
              <strong>
                {removePerson.firstName} {removePerson.lastName}
              </strong>{" "}
              wird aus „{area.name}“ entfernt.
            </>
          }
          confirmLabel={
            <>
              <Trash2 className="mr-2 h-4 w-4" />
              Aus Bereich entfernen
            </>
          }
