...
import type { MarshalAreaConfigAreaInput, MarshalAreaConfigShiftInput, MarshalEvent, MarshalPostConfigInput, MarshalSectionConfigInput, MarshalStructurePreview, MarshalWorkspace } from "@/types/admin-marshals";

type Props = {
  workspace: MarshalWorkspace;
  canWrite: boolean;
  busy: boolean;
  onSavePosts: (posts: MarshalPostConfigInput[], sections?: MarshalSectionConfigInput[]) => Promise<boolean>;
...
export function MarshalConfigView({ workspace, canWrite, busy, onSavePosts, onSaveAreas, onReset, events, currentEvent, onPreviewStructure, onInitializeEvent }: Props) {
  ...
  const [sectionDrafts, setSectionDrafts] = useState<MarshalSectionConfigInput[]>([]);
  ...
  useEffect(() => {
    setSectionDrafts([...workspace.sections].sort((a, b) => a.sortOrder - b.sortOrder).map(({ code, name, leaderCode, leaderTargetStaff, sortOrder }) => ({ code, name, leaderCode, leaderTargetStaff: leaderTargetStaff ?? 2, sortOrder })));
    ...
    <div className="space-y-4">
      <Card className={isConfigured ? "border-emerald-200 bg-emerald-50/30" : "border-blue-200 bg-blue-50/30"}>
        <CardHeader className="p-4 sm:p-6">
          <CardTitle className="flex items-center gap-2 text-lg">{isConfigured ? <CheckCircle2 className="h-5 w-5 text-emerald-600" /> : <Copy className="h-5 w-5 text-blue-600" />}{isConfigured ? "Helferstruktur vorbereitet" : "Helferstruktur für diese Veranstaltung vorbereiten"}</CardTitle>
        </CardHeader>
        {!isConfigured && <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
          <>
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
              <label className="grid gap-1 text-xs font-medium text-slate-600">Vorlagenveranstaltung<select className="h-11 rounded-md border bg-white px-3 text-base font-normal sm:text-sm" value={sourceEventId} disabled={!canWrite || busy} onChange={(event) => { setSourceEventId(event.target.value); setStructurePreview(null); }}><option value="">Veranstaltung auswählen</option>{sourceEvents.map((event) => <option key={event.id} value={event.id}>{event.name}</option>)}</select></label>
            ...
            {sourceEvents.length === 0 && <p className="rounded-lg border border-dashed p-3 text-sm text-slate-500">Es ist noch keine andere Veranstaltung als Vorlage verfügbar.</p>}
      ...
      <Card>
        <CardHeader className="p-4 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <CardTitle>Abschnittsleitungen</CardTitle>
            {canWrite && <Button type="button" disabled={busy || sectionDrafts.some((section) => section.leaderTargetStaff < 1 || section.leaderTargetStaff > 20)} onClick={() => void onSavePosts(postDrafts, sectionDrafts)}><Save className="mr-2 h-4 w-4" />Speichern</Button>}
          </div>
        </CardHeader>
        <CardContent className="grid gap-3 p-4 pt-0 sm:grid-cols-2 sm:p-6 sm:pt-0 xl:grid-cols-4">
          {sectionDrafts.map((section, index) => (
            <label key={section.code} className="grid gap-1 rounded-lg border bg-slate-50 p-3 text-xs font-medium text-slate-600">
              <span className="text-sm font-semibold text-slate-900">{section.leaderCode} · {section.name}</span>
              Soll Abschnittsleitung
              <Input type="number" min={1} max={20} value={section.leaderTargetStaff} disabled={!canWrite} onChange={(event) => setSectionDrafts(sectionDrafts.map((item, itemIndex) => itemIndex === index ? { ...item, leaderTargetStaff: Math.min(20, Math.max(1, Number(event.target.value) || 1)) } : item))} />
            </label>
          ))}
        </CardContent>
      </Card>
