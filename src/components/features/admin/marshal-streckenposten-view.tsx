...
export function MarshalStreckenpostenView({ workspace, day, dayKey, canWrite, busy, targetMode, onTargetModeChange, onDayChange, onPersonOpen, onListSave, onSave, onReplace }: Props) {
  ...
  const people = workspace.people.filter((person) => {
    ...
    if (sortMode === "name") return a.lastName.localeCompare(b.lastName, "de", { sensitivity: "base" }) || a.firstName.localeCompare(b.firstName, "de", { sensitivity: "base" }) || a.helperNumber - b.helperNumber;
    if (sortMode === "helperNumber") return a.helperNumber - b.helperNumber;
    if (sortMode === "status") {
      const statusOrder: Record<MarshalCommitmentStatus, number> = { accepted: 0, pending: 1, tentative: 2, declined: 3, not_asked: 4 };
    ...
    <section className="min-w-0 bg-white" aria-labelledby="streckenposten-title">
      ...
      <div className="space-y-4 px-3 py-4 sm:px-4 xl:px-0">
        {mode === "list" ? <>
          ...
          <div className="flex justify-end"><label className="grid gap-1 text-xs font-medium text-slate-600">Sortierung<select className="h-11 min-w-52 rounded-md border bg-white px-3 text-base font-normal text-slate-950 sm:text-sm" value={sortMode} onChange={(event) => setSortMode(event.target.value as SortMode)}><option value="post">Nach Posten</option><option value="name">Nach Name (A→Z)</option><option value="helperNumber">Nach Helfernummer</option><option value="status">Nach Status</option></select></label></div>
