      ...
      <CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
        {mode === "list" ? <>
          ...
          <div className="overflow-hidden rounded-lg border bg-white text-sm">
            ...
            {people.map((person) => { const assignment = person.assignments.find((item) => item.dayId === day.id); const assignmentValue = assignment?.postId ? `post:${assignment.postId}` : "none"; const disabled = !canWrite || busy || !person.isActive || person.noDeployment; return <div key={person.id} className={cn("grid gap-2 border-b px-3 py-2 last:border-b-0 md:grid-cols-[5rem_minmax(12rem,1fr)_minmax(12rem,1fr)_10rem] md:items-center md:gap-3", (person.noDeployment || !person.isActive) && "bg-red-50 text-red-900")}><span className="hidden text-slate-500 md:block">{person.helperNumber}</span><div className="min-w-0"><button type="button" className="break-words text-left font-medium text-blue-700 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => onPersonOpen(person)}>{person.lastName}, {person.firstName}</button><span className="ml-1 text-xs text-slate-500 md:hidden">#{person.helperNumber}</span>{person.noDeployment ? <span className="ml-2 text-xs font-semibold">⚠ kein Einsatz</span> : !person.isActive && <span className="ml-2 text-xs font-semibold">Inaktiv</span>}{(assignment?.note ?? person.participation.note) && <span className="block truncate text-xs text-slate-500" title={assignment?.note ?? person.participation.note ?? undefined}>{assignment?.note ?? person.participation.note}</span>}</div><AssignmentSelect value={assignmentValue} workspace={workspace} day={day} targetMode={targetMode} currentPersonId={person.id} disabled={disabled} onChange={(value) => void onSave(person, assignment?.commitmentStatus ?? "pending", value)} /><StatusSelect value={assignment?.commitmentStatus ?? "not_asked"} disabled={disabled} onChange={(status) => void onSave(person, status, assignmentValue === "none" ? "" : assignmentValue)} /></div>; })}
          </div>
...
function StatusSelect({ value, disabled, onChange }: { value: MarshalCommitmentStatus; disabled: boolean; onChange: (status: MarshalCommitmentStatus) => void }) { return <select aria-label="Zusage" className="h-9 w-full min-w-0 rounded-md border bg-white px-2 text-sm" value={value} disabled={disabled} onChange={(event) => onChange(event.target.value as MarshalCommitmentStatus)}>{Object.entries(marshalStatusLabels).map(([status, label]) => <option key={status} value={status}>{label}</option>)}</select>; }
function AssignmentSelect({ value, workspace, day, targetMode, currentPersonId, disabled, onChange }: { value: string; workspace: MarshalWorkspace; day: MarshalDay; targetMode: PlanningTargetMode; currentPersonId: string; disabled: boolean; onChange: (value: string) => void }) {
  return <select aria-label="Streckenposten" className="h-9 w-full min-w-0 rounded-md border bg-white px-2 text-sm" value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)}><option value="none">Nicht eingeteilt</option>{workspace.posts.filter((post) => post.isActive).sort((a, b) => a.sortOrder - b.sortOrder).map((post) => {
