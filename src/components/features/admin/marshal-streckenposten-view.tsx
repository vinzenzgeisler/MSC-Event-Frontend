        ...
        {mode === "list" ? <>
          ...
          <div className="overflow-x-auto"><table className="min-w-[820px] w-full text-sm"><thead><tr className="border-b bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><th className="p-3">Nr.</th><th className="p-3">Name</th><th className="p-3">Posten</th><th className="p-3">Rolle</th><th className="p-3">Status</th><th className="p-3">Bemerkung</th></tr></thead><tbody>
            {leaderRows.map(({ section, leader, historicalLeaders, assignment, showMain }) => {
              const mainRow = <tr key={`leader-slot:${section.id}`} className="border-b bg-violet-50/50 align-top"><td className="p-3 font-semibold text-violet-700">{section.leaderCode}</td><td className="p-3">{leader ? <><button type="button" className="font-medium text-blue-700 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => onPersonOpen(leader)}>{leader.lastName}, {leader.firstName}</button>{!leader.isActive && <span className="ml-2 text-xs font-semibold text-red-700">Inaktiv</span>}</> : canWrite ? <select aria-label={`${section.leaderCode} besetzen`} className="h-10 min-w-52 rounded-md border bg-white px-2 text-sm" value="" disabled={busy} onChange={(event) => { const person = workspace.people.find((item) => item.id === event.target.value); if (person?.isActive && !person.noDeployment) void onSave(person, "accepted", `leader:${section.id}`); }}><option value="">Abschnittsleitung wählen …</option>{workspace.people.filter((person) => person.isActive && !person.noDeployment).map((person) => <option key={person.id} value={person.id}>{person.helperNumber} · {person.lastName}, {person.firstName}</option>)}</select> : "Nicht besetzt"}</td><td className="p-3 font-semibold">{section.leaderCode}</td><td className="p-3">Abschnittsleitung</td><td className="p-3">{leader ? <StatusSelect value={assignment?.commitmentStatus ?? "not_asked"} disabled={!canWrite || busy || !leader.isActive} onChange={(status) => void onSave(leader, status, `leader:${section.id}`)} /> : <StatusBadge status="not_asked" />}</td><td className="p-3 text-slate-500">{assignment?.note ?? "—"}</td></tr>;
          ...
          })}</tbody></table></div>{people.filter((person) => person.assignments.find((item) => item.dayId === day.id)?.role !== "section_leader").length === 0 && leaderRows.length === 0 && <p className="rounded-lg border border-dashed p-6 text-center text-sm text-slate-500">Keine Einträge entsprechen den Filtern.</p>}
