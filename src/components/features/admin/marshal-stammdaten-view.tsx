...
const EMPTY_DRAFT = { firstName: "", lastName: "", street: "", zip: "", city: "", birthdate: "", email: "", phone: "", shirtSize: "", activityAreas: [] as string[], note: "" };
...
export function MarshalStammdatenView({ workspace, canWrite, busy, onPersonOpen, onCreate, onDelete }: Props) {
  ...
  const nextHelperNumber = Math.max(0, ...workspace.people.map((person) => person.helperNumber)) + 1;

  async function createPerson() {
    const saved = await onCreate({ helperNumber: nextHelperNumber, firstName: draft.firstName.trim(), lastName: draft.lastName.trim(), street: draft.street.trim() || null, zip: draft.zip.trim() || null, city: draft.city.trim() || null, birthdate: draft.birthdate || null, email: draft.email.trim() || null, phone: draft.phone.trim() || null, shirtSize: draft.shirtSize.trim() || null, activityAreas: draft.activityAreas, note: draft.note.trim() || null });
