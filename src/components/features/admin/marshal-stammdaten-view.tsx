...
import { isMarshalEventAreaAssignment } from "@/components/features/admin/marshal-assignment-helpers";
...
const EMPTY_DRAFT = { firstName: "", lastName: "", street: "", zip: "", city: "", birthdate: "", email: "", phone: "", shirtSize: "", activityAreas: [] as string[], note: "", eventAssignment: "none", shiftId: "", commitmentStatus: "pending" as MarshalCommitmentStatus };
...
function buildEventAssignmentLabels(person: MarshalPerson, workspace: MarshalWorkspace): string[] {
  ...
  workspace.areaAssignments
    .filter((assignment) => assignment.participationId === person.participation.id)
    .forEach((assignment) => {
      const area = workspace.areas.find((item) => item.id === assignment.areaId);
      if (area && isMarshalEventAreaAssignment(area.areaType, assignment.commitmentStatus)) labels.push(area.name);
    });
  ...
  workspace.shiftAssignments
    ...
    .forEach((assignment) => {
      ...
      if (shift && area && isMarshalEventAreaAssignment(area.areaType, assignment.commitmentStatus)) labels.push(`${area.name} ${shift.label}`);
    });
...
export function MarshalStammdatenView({ workspace, canWrite, busy, onPersonOpen, onCreate, onDelete }: Props) {
  ...
  const selectedDraftAreaId = draft.eventAssignment.startsWith("area:") ? draft.eventAssignment.slice("area:".length) : "";
  const selectedDraftArea = workspace.areas.find((item) => item.id === selectedDraftAreaId);
  const selectedDraftAreaShifts = workspace.areaShifts
    .filter((shift) => shift.areaId === selectedDraftAreaId)
    .sort((left, right) => left.sortOrder - right.sortOrder || left.shiftDate.localeCompare(right.shiftDate));
  ...
  async function createPerson() {
    ...
    const initialAssignment: MarshalInitialEventAssignment = kind === "area" && id
      ? { kind: "area", areaId: id, shiftId: draft.shiftId || undefined, commitmentStatus: draft.commitmentStatus }
      : kind === "track" && id
  ...
  return <>
    <Card><CardHeader className="p-4 sm:p-6"><CardTitle>Helferstammdaten</CardTitle></CardHeader><CardContent className="space-y-4 p-4 pt-0 sm:p-6 sm:pt-0">
      <div className="rounded-xl border bg-slate-50 p-3"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[minmax(15rem,1.5fr)_repeat(3,minmax(10.5rem,1fr))_minmax(9rem,auto)] xl:gap-4"><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Suche<span className="relative min-w-0"><Input className="w-full min-w-0 pr-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, Nummer oder E-Mail" />{search && <button type="button" aria-label="Suche leeren" className="absolute right-0 top-0 flex h-10 w-10 items-center justify-center rounded text-slate-500 hover:bg-slate-100" onClick={() => setSearch("")}><X className="h-4 w-4" /></button>}</span></label><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Standardbereich<select className="h-10 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal" value={area} onChange={(event) => setArea(event.target.value)}><option value="all">Alle Standardbereiche</option>{areas.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Diese Veranstaltung<select className="h-10 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal" value={eventAssignment} onChange={(event) => setEventAssignment(event.target.value)}><option value="all">Alle Zuordnungen</option><option value="assigned">Mit Zuordnung</option><option value="unassigned">Noch ohne Zuordnung</option></select></label><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Stammdatenstatus<select className="h-10 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal" value={active} onChange={(event) => setActive(event.target.value)}><option value="all">Aktiv und inaktiv</option><option value="active">Nur aktiv</option><option value="inactive">Nur inaktiv</option></select></label><label className="flex min-h-10 items-center gap-2 self-end whitespace-nowrap rounded-md border bg-white px-3 text-sm"><input type="checkbox" checked={onlyNoDeployment} onChange={(event) => setOnlyNoDeployment(event.target.checked)} />Kein Einsatz</label></div>
      ...
      {canWrite && <details className="rounded-xl border bg-slate-50/60 p-4"><summary className="min-h-10 cursor-pointer py-2 font-medium"><Plus className="mr-2 inline h-4 w-4" />Neue Person</summary><div className="mt-4 space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{([[
        "firstName", "Vorname", "text"], ["lastName", "Nachname", "text"], ["street", "Straße und Hausnummer", "text"], ["zip", "PLZ", "text"], ["city", "Ort", "text"], ["birthdate", "Geburtstag", "date"], ["email", "E-Mail", "email"], ["phone", "Telefon", "tel"], ["shirtSize", "T-Shirt-Größe", "text"], ["note", "Bemerkung", "text"]] as const).map(([key, label, type]) => <label key={key} className={cn("grid gap-1 text-xs font-medium text-slate-600", key === "note" && "sm:col-span-2")}>{label}{key === "firstName" || key === "lastName" ? <span className="sr-only"> (Pflichtfeld)</span> : null}<Input type={type} value={draft[key]} required={key === "firstName" || key === "lastName"} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} /></label>)}</div><MarshalAreaMultiSelect areas={workspace.areas} value={draft.activityAreas} onChange={(activityAreas) => setDraft({ ...draft, activityAreas })} /><div className="grid gap-3 rounded-lg border bg-white p-3 sm:grid-cols-2 xl:grid-cols-3"><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Direkte Zuordnung in dieser Veranstaltung<select className="h-11 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal" value={draft.eventAssignment} onChange={(event) => setDraft({ ...draft, eventAssignment: event.target.value, shiftId: "" })}><option value="none">Noch keine Zuordnung</option><optgroup label="Streckenposten">{workspace.days.map((day) => <option key={day.id} value={`track:${day.id}`}>Streckenposten · {day.label}</option>)}</optgroup><optgroup label="Helferbereiche">{[...workspace.areas].sort((a, b) => a.sortOrder - b.sortOrder).map((item) => <option key={item.id} value={`area:${item.id}`}>{item.name}</option>)}</optgroup></select></label><label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Status der Zuordnung<select className="h-11 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal disabled:bg-slate-100" value={draft.commitmentStatus} disabled={draft.eventAssignment === "none"} onChange={(event) => setDraft({ ...draft, commitmentStatus: event.target.value as MarshalCommitmentStatus })}>{Object.entries(marshalStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{selectedDraftAreaShifts.length > 0 && <label className="grid min-w-0 gap-1 text-xs font-medium text-slate-600">Schicht (optional)<select className="h-11 w-full min-w-0 rounded-md border bg-white px-3 text-sm font-normal" value={draft.shiftId} onChange={(event) => setDraft({ ...draft, shiftId: event.target.value })}><option value="">Nur {selectedDraftArea?.name ?? "Bereich"} zuordnen</option>{selectedDraftAreaShifts.map((shift) => <option key={shift.id} value={shift.id}>{shift.label} · {new Intl.DateTimeFormat("de-DE", { weekday: "short", day: "2-digit", month: "2-digit" }).format(new Date(`${shift.shiftDate}T00:00:00`))}</option>)}</select></label>}</div><Button type="button" className="min-h-11 w-full sm:w-auto" disabled={busy || !draft.firstName.trim() || !draft.lastName.trim()} onClick={() => void createPerson()}>Person anlegen</Button></div></details>}
