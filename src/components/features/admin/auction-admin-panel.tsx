...
export function AuctionAdminPanel({ eventId, canWrite }: { eventId: string; canWrite: boolean }) {
  ...
  const checks = [
    ...
    { label: '21-Sekunden-Reel hinterlegt', done: Boolean(auction.videoS3Key || auction.videoUrl) },
  ...
  return <div className="space-y-5">
    ...
    <div className="rounded-lg border bg-white p-5 shadow-sm space-y-4">
      ...
      <div className="grid gap-4 rounded-md border border-dashed p-4 md:grid-cols-2"><label className="space-y-2"><span className="flex items-center gap-2 text-sm font-medium"><Upload className="h-4 w-4"/>Helmbild hochladen</span><Input type="file" accept="image/jpeg,image/png,image/webp" disabled={!canWrite || uploading !== null} className="h-auto py-2" onChange={(event) => void uploadMedia('image', event.target.files?.[0])}/><span className="block text-xs text-slate-500">JPG, PNG oder WebP · maximal 15 MB</span></label><label className="space-y-2"><span className="flex items-center gap-2 text-sm font-medium"><Upload className="h-4 w-4"/>21-Sekunden-Reel hochladen</span><Input type="file" accept="video/mp4,video/webm" disabled={!canWrite || uploading !== null} className="h-auto py-2" onChange={(event) => void uploadMedia('video', event.target.files?.[0])}/><span className="block text-xs text-slate-500">MP4 oder WebM · maximal 250 MB</span></label>{uploading && <p className="text-sm font-medium text-blue-700 md:col-span-2">{uploading === 'image' ? 'Helmbild' : 'Reel'} wird hochgeladen…</p>}</div>
