...
const removeGlobalManifestLinkPlugin = (): Plugin => ({
  name: "remove-global-pwa-manifest-link",
  writeBundle() {
    ...
    const next = source.replace(/<link rel="manifest"[^>]*>\s*/g, "");
    if (next !== source) {
    ...
    plugins: [
      ...
      ...(enablePwa
        ? [
            ...
            removeGlobalManifestLinkPlugin()
          ]
        : [])
    ],
    resolve: {
